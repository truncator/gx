...
struct Input
{
    //
    // 00000000 -> nothing
    // 00000001 -> key down new
    // 00000011 -> key held
    // 00000010 -> key was down
    //
    ...
    uint8 mouse_buttons[3];
    vec2  mouse_down_positions[3];
};

void scroll_wheel_callback(struct GLFWwindow *window, double dx, double dy);
...
void wrap_cursor(struct GLFWwindow *window, struct Input *input, uint32 width, uint32 height);
